			...
			<?php for ($i = 0; $i < $rows_count; $i++) { ?>
				<tr>
					<form action="/admin/setting" method="post">
						...
						<td>
							<?php if (is_array($rows[$i]['value'])) : ?>
								<?php echo json_decode($rows[$i]['value']); ?>
							<?php else: ?>
								<?php echo $rows[$i]['value']; ?>
							<?php endif; ?>
							<br>

							<p style="margin-top: 8px !important;">
								<button type="button" value="ویرایش" style="background: none; border: none; color: blue;text-decoration: none; cursor: pointer;" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#exampleModal">ویرایش</button>
							</p>
